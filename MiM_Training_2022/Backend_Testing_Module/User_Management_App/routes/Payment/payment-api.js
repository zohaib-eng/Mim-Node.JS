...
const e = require('express');
      ...
      router.post('/addaccount',async(req,res,next)=>{
        ...
        var t=pay.email;
        const data=await Payment.findOne({email:req.body.email});
        if(data){
  ...
  router.post('/totalcreditbalance',async(req,res)=>{
    const{email,mimbalance}=req.body;
    const data = await validatemimbalance(email,mimbalance,res);
    // res.send(pay2);
    return res.status(201).json({
      success:true,
      data
...
const validatemimbalance=async(email,mimbalance,req)=>{
  const pay=await Payment.findOne({email});
  if(!pay){
  ...
  const oldRecord=await Payment.findOne({email:email});
  var e,f;
  ...
  const nxt7=await Payment.findOne({email:email});
        return nxt7;
}
...
// get payment data
  router.get('/checkbalance',async(req,res,next)=>{
    Payment.findOne(({email:req.query.email}),function(err,data){
        if(err){
          res.send(err);
        }
        return res.status(201).json({
...
router.post('/amountcutbalance',auth,async(req,res,next)=>{
    ...
    //   companyshare=e
      ...
      const nxt4=await Payment.findOneAndUpdate(
          {email},
            {$set: {'mimbalance': mimbalance,'impactei':f,companyshare:e,'gift':gift}}     
        )
      // const nxt10=await Impact.findOne({email:req.body.email});
      // sendMail({
      //   to: nxt10.email,
      //   OTP:otp.otp_code,
      // });
