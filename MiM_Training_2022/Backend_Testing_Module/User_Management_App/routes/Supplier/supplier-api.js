...
const {dispute:dispute}=require('../student/schema');
const {easymodel:easymodel}=require('../student/schema');
...
const uuid = require('uuid');
  ...
  router.put('/supplier/update',auth2,async(req,res)=>{
    ...
    router.put('/registerStep3',async (req, res) => {
 ...
 router.post('/SupplierSendOtpLogin',async(req,res)=>{
    try{
      email =req.body.email;
    ...
    router.post('/compareSuppOtp',async(req,res)=>{
        try{
          otp=req.body.otp;
...
router.post('/dispute',async(req,res)=>{
    var c=uuid.v4();
    const data = new dispute({
      uid:c,
      pendingpayments:req.body.pendingpayments,
      email:req.body.email,
      message:req.body.message
    });
    const oldEmail = await dispute.findOne({email:req.body.email });
    if(oldEmail){ 
    ...
    try{
      ...
      return [false, 'Unable to send message, Please try again later', error];
    }
  });
  ...
  //////////////////////
router.post('/easypasa',async(req,res)=>{
  const data = new easymodel({
    easyaccount:req.body.easyaccount,
    accounttitle:req.body.accounttitle,
  })
  const data1 = await easymodel.findOne({easyaccount:req.body.easyaccount})
  if(data1){
    return res.status(201).json({
      success : false,
  ...
  try{
    ...
    return[false,'unable to sign up', error]
  }
})

///////////////////////
router.put('/updateeasypasa',async(req,res)=>{
  const fetchaccount = req.body.easyaccount;
  const data=await easymodel.findOneAndUpdate({easyaccount:fetchaccount},req.body,{new:true});
  if(data){
    return res.status(201).json({
      success:true, 
      status:true,
...
router.get('/geteasypasa',async(req,res)=>{
  //const fetchaccount=req.body.easyaccount;
